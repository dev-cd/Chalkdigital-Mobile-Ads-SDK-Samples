...
import com.chalkdigital.android.cdads_sample.R;

/**
 * Created by arungupta on 28/06/16.
 */

public class CDEditText extends RelativeLayout implements View.OnClickListener, TextWatcher, EditText.OnFocusChangeListener {
    ...
    private CDEditTextListener listener;
    ...
    public CDEditText(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.cdEditTextStyle);
    }
    ...
    public CDEditText(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        inflater.inflate(R.layout.compound_cd_edittext, this, true);
    }
...
//        iconImageView.setImageDrawable(getResources().getDrawable(getResources().getIdentifier(screenName.toLowerCase() + "_icon_" + index, "drawable", getContext().getPackageName())));
    ...
    public CDEditTextListener getListener() {
        return listener;
    }

    public void setListener(CDEditTextListener listener) {
        this.listener = listener;
    }

    public void initialise(int index, boolean showOptional, boolean showClearButton, CDEditTextListener listener, String screenName) {
        setIndex(index);
